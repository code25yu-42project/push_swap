...
int	ft_newadd_lst(t_fd_record **rcdlst_ptr, t_fd_record **fdlst_ptr, int fd)
{
	...
	*fdlst_ptr = (t_fd_record *)malloc(sizeof(t_fd_record));
	if (*fdlst_ptr == NULL)
		return (-1);
	(*fdlst_ptr)->fd = fd;
	(*fdlst_ptr)->readrt = 1;
...
int	ft_parse_rcd(t_fd_record **fdlst_ptr, char **res_ptr, int reslen)
{
	...
	rcdlen = ft_strchr_pos((*fdlst_ptr)->record, '\0') - reslen;
...
int	ft_extract_line(t_fd_record **rcdlst_ptr, t_fd_record **fdlst_ptr, char **res_ptr)
{
	...
	ft_strchr_pos((*fdlst_ptr)->record, '\0'))
		return (0);
	reslen = ft_strchr_pos((*fdlst_ptr)->record, '\0');
	if (reslen == -1)
		reslen = ft_strchr_pos((*fdlst_ptr)->record, '\0');
