...
int	ft_malloc_join_rcd(t_fd_record **fdlst_ptr, int rcdlen, int buflen)
{
	char	*join_rcd;
...
int	ft_record(t_fd_record **fdlst_ptr, char *buf, int rcdlen, int buflen)
{
	...
	if (*fdlst_ptr != NULL && (*fdlst_ptr)->record != NULL)
		rcdlen = ft_strchr_pos((*fdlst_ptr)->record, '\0');
...
char	*get_next_line(int fd)
{
	...
	ft_extract_line(&rcdlst, &fdlst, &res);
	ft_freelst(&rcdlst, &fdlst, res);
	return (res);
}
